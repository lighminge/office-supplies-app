...
export default function ItemForm({ initialData, onSubmit, onCancel }: ItemFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white p-6 rounded-3xl shadow-sm border-2 border-pink-100">
      <h2 className="text-xl font-bold text-gray-700 mb-4 flex items-center gap-2">
        {initialData ? <Icons.Pencil className="w-5 h-5 text-pink-400" /> : <Icons.PlusCircle className="w-5 h-5 text-pink-400" />}
      ...
      <div>
        ...
        <input 
          {...register('name')} 
          className={cn("w-full rounded-2xl border-2 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-pink-200 transition-colors", errors.name ? "border-red-300" : "border-pink-100 focus:border-pink-300")}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select 
            {...register('category')} 
            className="w-full rounded-2xl border-2 border-pink-100 px-4 py-2 focus:outline-none focus:border-pink-300 focus:ring-2 focus:ring-pink-200 bg-white"
          >
        ...
        <div>
           ...
           <div className="flex items-center gap-2">
             <div className="p-2 bg-pink-50 rounded-xl">
               {selectedIcon && React.createElement((Icons as any)[selectedIcon] || Icons.HelpCircle, { className: 'w-6 h-6 text-pink-500' })}
             </div>
             <select 
              {...register('iconName')} 
              className="flex-1 rounded-2xl border-2 border-pink-100 px-4 py-2 focus:outline-none focus:border-pink-300 bg-white"
             >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input 
            ...
            className={cn("w-full rounded-2xl border-2 px-4 py-2 focus:outline-none focus:border-pink-300 focus:ring-2 focus:ring-pink-200", errors.quantity ? "border-red-300" : "border-pink-100")}
          />
        ...
        <div>
          ...
          <input 
            ...
            className={cn("w-full rounded-2xl border-2 px-4 py-2 focus:outline-none focus:border-pink-300 focus:ring-2 focus:ring-pink-200", errors.minQuantity ? "border-red-300" : "border-pink-100")}
          />
      ...
      <div className="flex gap-3 pt-4">
        ...
        <button 
          type="submit" 
          className="flex-1 px-4 py-2 rounded-2xl bg-pink-400 text-white font-medium hover:bg-pink-500 transition-colors shadow-sm shadow-pink-200"
        >
