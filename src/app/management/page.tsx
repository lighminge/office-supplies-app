...
export default function ManagementPage() {
  ...
  const handleAddProcItem = () => {
    ...
    setProcSupplyId(''); setProcQty(1); setProcPrice(0);
  };
  ...
  const handleAddBulkProcItems = () => {
    ...
    }, '確認加入', 'bg-orange-500 hover:bg-orange-600');
  };
  ...
  const openRestockModal = (proc?: ProcurementRecord) => {
    ...
    else { if (selectedProcs.length === 0) return alert('請先勾選要入庫的採購單！'); setTargetProcForRestock(null); }
    ...
    <main className="min-h-screen p-6 md:p-12 max-w-7xl mx-auto">
      ...
      {restockDateModalOpen && (
        <div className="fixed inset-0 bg-sky-100/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full border-2 border-sky-100 shadow-xl">
      ...
      <div className="flex gap-4 mb-6 overflow-x-auto pb-6 pt-3 px-2 custom-scrollbar">
        <button onClick={() => setActiveTab('supplies')} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold transition-colors ${activeTab === 'supplies' ? 'bg-sky-500 text-white shadow-md' : 'bg-white text-sky-500 hover:bg-sky-50 border border-sky-100'}`}><Package className="w-5 h-5" /> 物品管理</button>
        <button onClick={() => setActiveTab('categories')} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold transition-colors ${activeTab === 'categories' ? 'bg-sky-500 text-white shadow-md' : 'bg-white text-sky-500 hover:bg-sky-50 border border-sky-100'}`}><Tag className="w-5 h-5" /> 物品類別</button>
        <button onClick={() => setActiveTab('icons')} className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-bold transition-colors ${activeTab === 'icons' ? 'bg-sky-500 text-white shadow-md' : 'bg-white text-sky-500 hover:bg-sky-50 border border-sky-100'}`}><ImageIcon className="w-5 h-5" /> 插圖管理</button>
        <button onClick={() => setActiveTab('procurement')} className={`relative flex items-center gap-2 px-6 py-3 rounded-2xl font-bold transition-colors ${activeTab === 'procurement' ? 'bg-sky-500 text-white shadow-md' : 'bg-white text-sky-500 hover:bg-sky-50 border border-sky-100'}`}>
        ...
        {activeTab === 'procurement' && (
          <div className="animate-in fade-in duration-300">
            <div className="bg-orange-50 border-2 border-orange-100 rounded-3xl p-6 mb-8 shadow-sm">
              ...
              <div className="bg-white rounded-2xl border border-orange-100 overflow-hidden mb-6 shadow-sm"><div className="flex bg-orange-100/50"><button onClick={() => setProcAddMode('single')} className={`flex-1 py-3 text-sm font-black ${procAddMode === 'single' ? 'bg-white text-orange-600 border-t-4 border-orange-500' : 'text-gray-500'}`}>單筆加入</button><button onClick={() => setProcAddMode('bulk')} className={`flex-1 py-3 text-sm font-black ${procAddMode === 'bulk' ? 'bg-white text-orange-600 border-t-4 border-orange-500' : 'text-gray-500'}`}>批次加入品項</button></div>
                <div className="p-5">{procAddMode === 'single' ? (
                  <div className="flex flex-col md:flex-row gap-3 items-end"><div className="flex-1 space-y-1"><label className="text-[10px] font-bold text-gray-400">物品類別</label><select value={procCatId} onChange={e => {setProcCatId(e.target.value); setProcSupplyId('');}} className="w-full rounded-xl border-2 border-orange-100 px-3 py-2 outline-none">{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="flex-[2] space-y-1"><label className="text-[10px] font-bold text-gray-400">物品名稱</label><select value={procSupplyId} onChange={e => { setProcSupplyId(e.target.value); const s = supplies.find(sup => sup.id === e.target.value); if(s) setProcPrice(s.price || 0); }} className="w-full rounded-xl border-2 border-orange-100 px-3 py-2 outline-none">{procFilteredSupplies.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div><div className="w-24 space-y-1"><label className="text-[10px] font-bold text-gray-400">數量</label><input type="number" value={procQty} onChange={e => setProcQty(e.target.value === '' ? '' : parseInt(e.target.value))} className="w-full rounded-xl border-2 border-orange-100 px-3 py-2 text-center" /></div><div className="w-24 space-y-1"><label className="text-[10px] font-bold text-gray-400">單價($)</label><input type="number" value={procPrice} onChange={e => setProcPrice(e.target.value === '' ? '' : parseInt(e.target.value))} className="w-full rounded-xl border-2 border-orange-100 px-3 py-2 text-center" /></div><button onClick={handleAddProcItem} className="bg-orange-500 text-white px-6 py-2 rounded-xl font-bold hover:bg-orange-600 transition-colors h-[44px]">儲存品項</button></div>
                ) : (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-48 overflow-y-auto pr-1">{procFilteredSupplies.map(s => (<label key={s.id} className={`flex flex-col p-2 rounded-xl border-2 cursor-pointer ${bulkSelectedSupplies.includes(s.id) ? 'bg-orange-50 border-orange-400' : 'bg-white border-gray-50'}`}><div className="flex gap-1 items-start"><input type="checkbox" checked={bulkSelectedSupplies.includes(s.id)} onChange={e => { if(e.target.checked) setBulkSelectedSupplies([...bulkSelectedSupplies, s.id]); else setBulkSelectedSupplies(bulkSelectedSupplies.filter(id => id !== s.id)); }} /><span className="text-xs font-bold truncate">{s.name}</span></div><div className="text-[10px] text-gray-400 mt-1">單價: ${s.price} | 庫存: {s.quantity}</div></label>))}</div></div>
                )}</div></div>
              {procItems.length > 0 && (<div className="bg-white rounded-2xl p-5 border border-orange-100 mb-6 shadow-sm"><table className="w-full text-left text-sm"><thead><tr className="border-b text-gray-400"><th className="pb-2 w-10">序號</th><th className="pb-2">名稱</th><th className="pb-2">數量</th><th className="pb-2">單價</th><th className="pb-2">小計</th><th className="pb-2 text-right">操作</th></tr></thead><tbody>{procItems.map((it, i) => (<tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-orange-50/30"><td className="py-2 text-gray-400">{i+1}</td><td className="py-2 font-bold">{it.name}</td><td className="py-2 text-orange-500 font-black">x{it.quantity}</td><td className="py-2">${it.unitPrice}</td><td className="py-2 font-bold">${it.quantity*it.unitPrice}</td><td className="py-2 text-right"><button onClick={() => handleEditProcItem(it)} className="text-sky-500 p-1 mr-1"><Edit3 className="w-4 h-4"/></button><button onClick={() => handleRemoveProcItem(it.supplyId)} className="text-red-400 p-1"><Trash2 className="w-4 h-4"/></button></td></tr>))}</tbody></table><div className="flex justify-between items-end mt-4"><div className="text-xs text-gray-400 font-bold">品項總數: {procItems.length} 項</div><div className="text-2xl font-black text-orange-600">總金額: ${procItems.reduce((s, it) => s + (it.quantity*it.unitPrice), 0)}</div></div></div>)}
              <button onClick={handleSaveProcurement} disabled={procItems.length === 0} className="w-full bg-orange-500 hover:bg-orange-600 text-white py-4 rounded-2xl font-black text-xl shadow-lg transition-transform hover:scale-[1.01]">{editingHistoryProcId ? '儲存修改單據' : '儲存這筆採購單 ✨'}</button>{editingHistoryProcId && <button onClick={() => { setEditingHistoryProcId(''); setProcItems([]); }} className="w-full mt-3 bg-gray-100 py-3 rounded-2xl font-bold">取消修改</button>}
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6 bg-gray-50 p-4 rounded-xl border border-gray-100 items-center"><div className="flex items-center gap-2"><span className="text-sm font-bold">查詢：</span><input type="date" value={procStartDate} onChange={e => setProcStartDate(e.target.value)} className="rounded-lg border px-2 py-1" /><span className="text-gray-400">~</span><input type="date" value={procEndDate} onChange={e => setProcEndDate(e.target.value)} className="rounded-lg border px-2 py-1" /><select value={procHistoryStatusFilter} onChange={e => setProcHistoryStatusFilter(e.target.value)} className="rounded-lg border px-2 py-1 text-sm"><option value="All">全部狀態</option><option value="pending">待入庫</option><option value="restocked">已入庫</option></select></div><div className="flex-1"></div><button onClick={() => openRestockModal()} disabled={selectedProcs.length === 0} className="bg-green-500 text-white px-4 py-2 rounded-xl font-bold text-sm disabled:opacity-50 flex items-center gap-1"><Package className="w-4 h-4"/> 批次入庫 ({selectedProcs.length})</button></div>
            <div className="overflow-x-auto bg-white rounded-xl shadow-sm border border-sky-100"><table className="w-full text-left text-sm border-collapse"><thead><tr className="bg-sky-50 text-sky-700"><th className="py-3 pl-4 rounded-tl-xl w-16"><div className="flex flex-col items-start gap-1"><button onClick={() => setSelectedProcs(paginatedProcs.filter(p => !p.isRestocked).map(p => p.id))} className="text-[10px] bg-sky-200 px-1 rounded">全選</button><input type="checkbox" onChange={e => { if(e.target.checked) setSelectedProcs(paginatedProcs.filter(p => !p.isRestocked).map(p => p.id)); else setSelectedProcs(selectedProcs.filter(id => !paginatedProcs.find(p => p.id === id))); }} checked={paginatedProcs.length > 0 && paginatedProcs.filter(p => !p.isRestocked).every(p => selectedProcs.includes(p.id))} className="w-4 h-4 ml-1" /></div></th><th className="py-3 font-bold text-center">序號</th><th className="py-3 font-bold">單號</th><th className="py-3 font-bold">日期/地點</th><th className="py-3 font-bold">金額</th><th className="py-3 font-bold text-right pr-4">操作</th></tr></thead><tbody>{paginatedProcs.map((p, i) => (<tr key={p.id} className="border-b last:border-0 hover:bg-sky-50/50"><td className="py-3 pl-5">{!p.isRestocked && <input type="checkbox" checked={selectedProcs.includes(p.id)} onChange={e => { if(e.target.checked) setSelectedProcs([...selectedProcs, p.id]); else setSelectedProcs(selectedProcs.filter(id => id !== p.id)); }} className="w-4 h-4" />}</td><td className="py-3 text-center text-gray-400">{(procPage-1)*procPerPage+i+1}</td><td className="py-3 font-bold text-gray-800">{p.id}</td><td className="py-3"><div>{p.date}</div><div className="text-[10px] text-gray-400">{p.location}</div></td><td className="py-3"><div className="font-bold text-orange-500">${p.totalAmount}</div>{p.isRestocked ? <span className="text-[10px] bg-green-100 text-green-600 px-1.5 rounded">已入庫</span> : <span className="text-[10px] bg-yellow-100 text-yellow-600 px-1.5 rounded">待入庫</span>}</td><td className="py-3 text-right pr-4"><div className="flex justify-end gap-1">{!p.isRestocked && <><button onClick={() => handleEditProcurementHistory(p)} className="p-1.5 bg-sky-50 text-sky-600 rounded-lg"><Edit3 className="w-4 h-4"/></button><button onClick={() => openRestockModal(p)} className="p-1.5 bg-green-50 text-green-600 rounded-lg"><Package className="w-4 h-4"/></button></>}<button onClick={() => handleDeleteProcurement(p.id)} disabled={p.isRestocked} className="p-1.5 bg-red-50 text-red-500 rounded-lg disabled:opacity-30"><Trash2 className="w-4 h-4"/></button></div></td></tr>))}</tbody></table></div>{totalProcPages > 1 && (<div className="flex justify-center items-center gap-4 mt-6"><button disabled={procPage === 1} onClick={() => setProcPage(p => p - 1)} className="px-3 py-1.5 bg-sky-50 rounded-lg font-bold disabled:opacity-50">上頁</button><span className="text-sm font-bold">{procPage} / {totalProcPages}</span><button disabled={procPage === totalProcPages} onClick={() => setProcPage(p => p + 1)} className="px-3 py-1.5 bg-sky-50 rounded-lg font-bold disabled:opacity-50">下頁</button></div>)}
          </div>
        )}
      </div>
